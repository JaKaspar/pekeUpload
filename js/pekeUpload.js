 ...
 *  Modified work Copyright (c) 2019 JaKaspar
...
(function($) {
    ...
    $.fn.pekeUpload = function(options) {
        ...
        var defaults = {
            ...
            classPreview: 'col-lg-3 col-md-3 col-xs-5',
            classPreviewLimitSize: false,
            classFinfo: 'col-lg-9 col-md-9 col-xs-9',
        ...
        var pekeUpload = {
            ...
            initImage: function(file) {
                ...
                if (typeof file !== "undefined") {
                    ...
                    reader.onload = function(e) {
                        img.onload = function() {
                            if (options.maxDimensionsWH > 0) {
                                if (img.width <= options.maxDimensionsWH && img.height <= options.maxDimensionsWH) {
                                    return d.resolve();
                                } else {
                    ...
                    return d.reject("Image not loaded");
                }
                return d.promise();
            },
            ...
            checkFile: function(file) {
                ...
                this.initImage(file)
                    .then(
                        function() {
                            pekeUpload.checkFileSetDimCheckResult(true, file);
                        },
                        function() {
                            pekeUpload.checkFileSetDimCheckResult(false, file);
                        }
                    );

            },
            checkFileSetDimCheckResult: function(sizeOk, file) {
                this.imgSizeOk = sizeOk;
                this.checkFileContinue(file);
            },
            checkFileContinue: function(file) {
                var pos = uploadCount;
                ...
                switch (options.bootstrap) {
                    case true:
                        ...
                        if (options.showPreview) {
                            var prev = $('<div class="' + options.classPreview + '"></div>').appendTo(newRow);
                            this.previewFile(prev, file);
                        }
                        var finfo = $('<div class="' + options.classFinfo + '"></div>').appendTo(newRow);
                        if (options.showFilename) {
            ...
            previewFile: function(container, file) {
                ...
                switch (type) {
                    case "image":
                        ...
                        if (options.classPreviewLimitSize) {
                            var prev = $('<img class="thumbnail" src="' + fileUrl + '" style="height:64px;max-height:64px;max-width:64px;" />').appendTo(container);
                        } else {
                            var prev = $('<img class="thumbnail" src="' + fileUrl + '" style="height:64px" />').appendTo(container);
                        }
                        break;
